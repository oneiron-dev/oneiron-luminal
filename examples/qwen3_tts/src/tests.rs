...
fn test_main_model_weight_names() {
    ...
    let path =
        Path::new("/home/ubuntu/projects/luminal-qwen/Qwen3-TTS-12Hz-1.7B-VoiceDesign/model.safetensors");
...
fn test_speech_decoder_weight_names() {
    ...
    let path = Path::new("/home/ubuntu/projects/luminal-qwen/Qwen3-TTS-Tokenizer-12Hz/model.safetensors");
    let data = std::fs::read(path)
        .unwrap_or_else(|e| panic!("Failed to read {}: {e}", path.display()));
    let tensors = SafeTensors::deserialize(&data)
        .unwrap_or_else(|e| panic!("Failed to parse {}: {e}", path.display()));
